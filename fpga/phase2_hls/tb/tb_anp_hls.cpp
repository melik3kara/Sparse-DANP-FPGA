// Phase-2 HLS C-simulation testbench.
//
// Drives BOTH kernels (anp_original_forward, anp_linearized_delta) against BOTH
// Phase-0 golden fixtures (nominal + branch_crossing), comparing every kernel
// output to its golden binary at absolute tolerance 1e-6.
//
// Returns non-zero if any comparison exceeds the tolerance or any file fails to
// load -> HLS csim reports failure. Host-only file I/O (allowed in the TB).
//
// GOLDEN_ROOT is injected by the Tcl via -cflags (absolute path to fpga/phase0).
#include "anp_hls.hpp"
...
// Golden-fixture root resolved at RUNTIME from env var ANP_GOLDEN_ROOT (chosen
// because the repository path contains a space, which is fragile for -D/-I
// injection). Falls back to this default if the env var is unset.
...
// Load all kernel inputs from a fixture's bin directory. Returns false on error.
struct Inputs {
    float x[16];
...
// Exercise both kernels on one fixture.
void run_fixture(const std::string& fixture_subdir) {
    ...
    float a_dz0[8], a_dz1[4], a_dz2[2], a_clean[2], a_noisy[2];
    anp_original_forward(in.x, in.W0, in.b0, in.eps0, in.W1, in.b1, in.eps1,
                         in.W2, in.b2, in.eps2,
                         a_dz0, a_dz1, a_dz2, a_clean, a_noisy);
    std::printf("  --- A. anp_original_forward ---\n");
    compare(dir, "dz_original_0", a_dz0, 8);
    compare(dir, "dz_original_1", a_dz1, 4);
    compare(dir, "dz_original_2", a_dz2, 2);
    compare(dir, "clean_logits",  a_clean, 2);
    compare(dir, "noisy_logits",  a_noisy, 2);

    // ---- B. Dense linearized ANP ----
    float b_dz0[8], b_dz1[4], b_dz2[2], b_clean[2], b_approx[2];
    anp_linearized_delta(in.x, in.W0, in.b0, in.eps0, in.W1, in.b1, in.eps1,
                         in.W2, in.b2, in.eps2,
                         b_dz0, b_dz1, b_dz2, b_clean, b_approx);
    std::printf("  --- B. anp_linearized_delta ---\n");
    compare(dir, "dz_linearized_0", b_dz0, 8);
    compare(dir, "dz_linearized_1", b_dz1, 4);
    compare(dir, "dz_linearized_2", b_dz2, 2);
    compare(dir, "clean_logits",    b_clean, 2);
    compare(dir, "approx_logits",   b_approx, 2);
}

} // namespace

int main() {
    std::printf("Phase-2 HLS C-simulation: both kernels vs Phase-0 golden fixtures\n");
