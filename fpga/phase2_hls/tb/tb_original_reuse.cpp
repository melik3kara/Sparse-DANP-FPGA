// Phase-2 HLS per-kernel testbench: anp_original_reuse_l0 ONLY.
//
// Kernel C reproduces the ORIGINAL noisy-forward ANP semantics exactly (it only
// reuses the clean layer-0 preactivation instead of recomputing x @ W0 + b0), so it
// is validated against the SAME original-ANP golden outputs as kernel A:
//   dz_original_0/1/2, clean_logits, noisy_logits.
//
// Single-kernel testbench (required for RTL co-simulation, which wraps one top).
// Drives both Phase-0 fixtures (nominal + branch_crossing) at absolute tolerance
// 1e-6; returns non-zero on any mismatch or load failure (tolerance never relaxed).
//
// Golden root resolved at RUNTIME from ANP_GOLDEN_ROOT (absolute path -> cosim cwd
// independent). Host-only file I/O (allowed in the TB).
#include "anp_hls.hpp"
...
// Exercise the exact-reuse kernel on one fixture, compared to ORIGINAL golden.
void run_fixture(const std::string& fixture_subdir) {
    ...
    float dz0[8], dz1[4], dz2[2], clean[2], noisy[2];
    anp_original_reuse_l0(in.x, in.W0, in.b0, in.eps0, in.W1, in.b1, in.eps1,
                          in.W2, in.b2, in.eps2,
                          dz0, dz1, dz2, clean, noisy);
    std::printf("  --- anp_original_reuse_l0 (vs original golden) ---\n");
    compare(dir, "dz_original_0", dz0, 8);
    compare(dir, "dz_original_1", dz1, 4);
    compare(dir, "dz_original_2", dz2, 2);
    compare(dir, "clean_logits",  clean, 2);
    compare(dir, "noisy_logits",  noisy, 2);
}

} // namespace

int main() {
    std::printf("Phase-2 HLS per-kernel TB: anp_original_reuse_l0 vs ORIGINAL Phase-0 golden\n");
